...
def main(count, collection, name, time, threshold_toxic, threshold_s_toxic):
    source = []
    target = []
    features = []
    graphs = []
    ...
    for idx, item in enumerate(collection.find({"detoxify-original": True, "detoxify-original-average":{ "$exists": True}, "lang" : "Language.ENGLISH"}).limit(count).allow_disk_use(True)):
        ...
        toxic = 0
        if len(item["log"]["chat"]) != 0:
            if item["detoxify-original-average"] > threshold_toxic:
                toxic = 2
            elif item["detoxify-original-average"] > threshold_s_toxic:
                toxic = 1
        x = torch.tensor(nodes, dtype=torch.float32)
        graphs.append(Data(x=x, edge_attr=torch.tensor(features, dtype=torch.float32), edge_index=torch.tensor([source, target], dtype=torch.long),y=torch.tensor([toxic]).to(torch.long)))
        source = []
        nodes=[]
        target = []
        features = []
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument("--threshold_toxic", help="Optional toxicity threshold value", type=float, default=0.3)
    arg_parser.add_argument("--threshold_slightly", help="Optional toxicity threshold value", type=float, default=0.2)
    ...
    main(count, collection, args.name, args.time, args.threshold_toxic, args.threshold_slightly)
    print("Done.")